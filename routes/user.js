...
var storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    console.log(file, "this is file in storage")
...
router.post('/api/signup', function (req, res) {
  ...
  if (!req.body.name || !req.body.password || !req.body.email) {
    ...
    var code;
    if(req.body.permission.includes('Sales Executive') || req.body.permission.includes('Merchant') || req.body.permission.includes('Promoter')) {
              ...
              if(data) {
                ...
                data.save(function(err) {
                  if(err) {
                    ...
                    console.log("Successfully increased ref bonus")
                  }
                })
              } else {
...
router.post('/api/login', (req, res) => {
  ...
  User.findOne(query, function (err, user) {
    ...
    if (err) {
      // console.log(err);
      response.err = "Wrong Credentials";
      response.success = false;
      return res.status(200).json(response);
      //  res.status(500).send({success: false, msg: 'Internal Server Error.'});
    } else {
    if (!user) {
      response.err = 'Authentication failed. User not found.';
      response.success = false;
      return res.status(200).json(response);
      ...
      console.log(user);
      if(user.permission.includes('Admin') && req.body.app != 1 || !user.permission.includes('Admin') && req.body.app == 1) {
      user.comparePassword(req.body.password, function (err, isMatch) {
        ...
        if (isMatch && !err) {
          ...
          User.findByIdAndUpdate(user._id, update_login_date, function (err, post) {
            if (err) {
              response.err = 'Authentication failed. Wrong creditinals.' + err;
              response.success = false;
              res.status(200).json(response);
            } else {
              // console.log(post.permission)
              if((req.body.app &&  req.body.app == 1)  && (post.permission.includes('Merchant') || post.permission.includes('Sales Executive') || post.permission.includes('Promoter'))){
          ...
          response.err = 'Authentication failed. Wrong creditinals.';
          response.success = false;
          res.status(200).json(response);
        }
      });
      ...
      if(user.permission.includes('Admin')) {
        response.err = 'You are Admin. You are not allowed to login from App.'; 
      } else {
        response.err = 'You are not allowed to login from admin. Please login from app.';
      }
      response.success = false;
      res.status(200).json(response);
    }
...
router.get('/api/userlist', function (req, res) {
  ...
  User.find(dbquery, {deleted:0}).count().exec(function (err, totalCount) {
    ...
    User.find(dbquery, { name: 1, email: 1, _id: 1, deleted: 1, created: 1, permission: 1, login_date: 1, ref_code: 1, mobile: 1 })
...
if(req.query.first && req.query.second) {
  var fdate = new Date(req.query.first).getTime();
  var sdate = new Date(req.query.second).getTime() + 60 * 60 * 24 * 1000;
  dbquery.created = { "$gte": fdate, "$lte": sdate};
}
  /******* pagination query started here ***********/
  var pageNo = parseInt(req.query.pageNo)  //req.query.pageNo
  var size = 10  //
                          ...
                          <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
                            ...
                            <tr>
                              <td class="text-button2 section" style="color:#613b51; font-family:Arial,sans-serif, 'Roboto'; font-size:14px; line-height:22px; text-align:center; padding:10px 25px 10px 25px">
                                <br/>
                                <a href="http://biznezupaaye.com/resetpassword/${token}" target="_blank" class="link3" style="color:#ffffff; text-decoration:none;background:#3696c2;padding:10px 15px 10px 15px;"><span class="link3" style="color:#ffffff; text-decoration:none">Click Here to Reset Password</span></a>
                                <br/><br/>
                              </td>
                            </tr>
                            <tr>
                      ...
                      <tr>
                        <td>
                          ...
                          <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
                            <tr>
                              <td class="section" style="padding:30px 250px 30px 250px; text-align:center">
                                <div class="fluid-img" style="font-size:0pt; line-height:0pt; text-align:left"><img src="cid:logo" border="0" width="150" height="60" alt="" /></div>
                              </td>
                            </tr>
                          </table>
                          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="spacer" style="font-size:0pt; line-height:0pt; text-align:center; width:100%; min-width:100%"><tr><td height="3" class="spacer" style="font-size:0pt; line-height:0pt; text-align:center; width:100%; min-width:100%">&nbsp;</td></tr></table>
...
router.route('/api/otp').post(function(req, res) {
  ...
  if(!mobile || !otp || mobile.length < 10 || !req.body.email)  {
    res.status(200).json({
      success: false,
      msg: "Please enter 10 digit mobile number and email"
    })
  } else {
    //var url = `http://103.247.98.91/API/SendMsg.aspx?uname=20190444&pass=929a99ZH&send=BIZUPY&dest=${mobile}&msg=Thanks for Registering with us. Your OTP is ${otp}. Do not share this OTP with anyone. Best Regards, BiznezUpaay`;
    var url = "";
                    ...
                    <tr>
                      <td>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
                          <tr>
                            <td class="section" style="padding:40px 25px 5px 25px;font-size:13px;color:#000000;">
                              <b style="font-size:14px;color:#007bdb;">Thankyou for registering with us.</b><br/><br/>
                              Your new OTP is ${otp}<br/><br/>
...
router.post('/api/user/upload', upload.single('doc'), function (req, res, next) {
  // req.file is the `avatar` file
  
  var url = `http://biznezupaaye.com/app2api/file/${req.file.originalname}`
  var obj = {
    url: url,
    user: res.locals.id
  };
  var newFile = new File(obj);
  newFile.save(function(err, data) {
    if(err) {
      res.status(200).json({
        success: false,
        msg: "Something went wrong. Please upload again",
        err: err.message
      })
    } else {
      res.status(200).json({
...
router.get('/api/user/files', function(req, res) {
  var id = res.locals.id;
  File.find({user: id}, function(err, data) {
    if(err) {
      res.status(200).json({
        success: false,
...
router.get('/api/file/:id', function(req, res) {
