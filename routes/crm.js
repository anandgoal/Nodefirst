...
router.route('/api/crm/delete')
      ...
      router.route('/api/crm/mail').post((req, res) => {
        var msg = req.body.msg;
        var id = res.locals.id;
        CrmModel.find({merchant_Id: id}, function(err, data) {
          if(err) {
            res.status(200).json({
              success: false,
              msg: err.message
            ...
            if(data.length > 0) {
              console.log(data, 'these are merchants');
              var msg_promises = [];
              data.forEach(function(user) {
                msg_promises.push(new Promise(function(resolve, reject) {
                  var url = ``
                  http.get(url, (resp) => {
                    let data = '';
              ...
              Promise.all(msg_promises).then(function() {
                res.status(200).json({
                  success: true,
                  msg: "Successfully sent msg to all users"
                })
              }).catch(function(err) {
                res.status(200).json({
                  success: false,
